...
if "%server%" == "1" (
    set serverName=Kronos
    goto realmMenuKronos1
)
if "%server%" == "2" (
    set serverName=Kronos III
    goto realmMenuKronos3
)
:realmMenuKronos1
cls
echo.
echo.
echo.                    Kronos 1 Configurator
echo.                    ----------------------------        
echo.
echo.           Select the area you want to play from. Each login server 
echo.           will connect you to the same realm, but the path that 
echo.           each server takes is geographically different. This means
echo.           that you can try each realmlist and see which one provides
echo.           you with the lowest latency.
echo.
echo.                      1. West
echo.                      2. North
echo.                      3. East
echo.
echo.
set /p realm=".%BS%          Enter area number (1-3): "
...
echo.
echo. 
echo.       Adding 'set realmlist %realmNAme%.kronos-wow.com' to realmlist.wtf
echo set realmlist %realmNAme%.kronos-wow.com > realmlist.wtf
echo.
...
echo. 
echo.       Adding 'SET realmList "%realmName%.kronos-wow.com"' to config.wtf
echo SET realmList "%realmName%.kronos-wow.com" >> config.wtf
echo.       Adding 'SET realmName "%serverName%"' to config.wtf
echo SET realmName "%serverName%" >> config.wtf
echo.
